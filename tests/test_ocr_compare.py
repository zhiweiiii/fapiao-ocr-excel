...
def run_case(pdf_path: Path, json_path: Path, paddleocr_manager) -> Dict[str, Any]:
    ...
    truth_main = truth.get('main') if isinstance(truth.get('main'), dict) else {k: v for k, v in truth.items() if k != 'items'}
    truth_items = truth.get('items') if isinstance(truth.get('items'), list) else truth.get('main', {}).get('items', [])
