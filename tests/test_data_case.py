...
# 复用主表和明细字段映射
KEYWORDS = getattr(main_mod, 'KEYWORDS')
ITEM_KEY_MAP = getattr(main_mod, 'ITEM_KEY_MAP')
MAIN_FIELD_MAP = getattr(main_mod, 'MAIN_FIELD_MAP')
DETAIL_FIELD_MAP = getattr(main_mod, 'DETAIL_FIELD_MAP')
...
def assert_main_row(ws, data: dict):
    ...
    # 动态核对关键字段（用 KEYWORDS 的键 + invoice_type，映射到中文表头）
    keys = list(KEYWORDS.keys()) + ['invoice_type']
    for k in keys:
        header = MAIN_FIELD_MAP.get(k, k)
        expected = data.get(k)
...
def assert_detail_rows(ws, items: List[dict]):
    # 用 ITEM_KEY_MAP 的值集合映射到中文表头
    item_keys = sorted(set(ITEM_KEY_MAP.values()))
    required_headers = [DETAIL_FIELD_MAP.get(k, k) for k in item_keys]
    assert_sheet_headers(ws, required_headers)
    # 数据行数量 = items 长度
...
# 计算主表字段准确率（复用 KEYWORDS 的字段集合，并补充 invoice_type）
def compute_main_accuracy(expected: dict, ocr: dict):
    keys = list(KEYWORDS.keys()) + ['invoice_type']
    keys = [k for k in keys if (k in expected) or (k in ocr)]
    total = len(keys)
    match = 0
    mismatches = []
    for k in keys:
...
def compute_items_accuracy(expected_items: list, ocr_items: list):
    ...
    total = n * len(ITEM_KEYS) if n else 0
    match = 0
    mismatches = []
    for i in range(n):
...
def run_case(json_path: Path):
    ...
    # 计算准确率（主表 + 明细）
    main_acc, main_mismatches, main_total = compute_main_accuracy(data, ocr_info)
    items_acc, items_mismatches, items_total = compute_items_accuracy(data.get('items', []), ocr_info.get('items', []))
    # 综合准确率按字段总数加权（使用返回的总字段数）
    ...
    ws_main = wb['发票主表']
    ws_detail = wb['发票明细']
    # 动态计算主表必需表头：KEYWORDS 的键 + invoice_type 映射到中文
    required_main_headers = [MAIN_FIELD_MAP.get(k, k) for k in (list(KEYWORDS.keys()) + ['invoice_type'])]
    assert_sheet_headers(ws_main, required_main_headers)
